...
export default function Navbar() {
  ...
  // Check if current route is About page for dark navy navbar matching
    ...
    <>
      <header 
        className={`sticky top-0 z-50 w-full transition-all duration-300 ${
          isAboutPage
            ? 'bg-[#0B1528] border-b border-blue-900/40 text-white h-14 sm:h-16'
            : isScrolled 
      ...
      >
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-full flex items-center justify-between gap-2">
          {/* Brand Logo - Extra Large & Full Navbar Fit */}
          <Link href="/" className="relative flex items-center shrink-0 group h-full">
            <Image 
              ...
              className={`h-16 sm:h-20 lg:h-24 w-auto max-w-none object-contain scale-[1.35] sm:scale-[1.45] origin-left transition-transform duration-200 group-hover:scale-[1.5] ${
                isAboutPage ? 'brightness-0 invert' : ''
              }`}
              priority
            />
          </Link>
          ...
          {/* Desktop Navigation */}
          <nav className={`hidden lg:flex items-center gap-0.5 xl:gap-1 p-1 rounded-2xl border text-xs xl:text-sm font-semibold shrink-0 ${
            isAboutPage
              ? 'bg-blue-950/80 border-blue-800/60 text-slate-200'
            ...
            >
              ...
              >
                <span>Services</span>
                <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${servicesOpen ? 'rotate-180 text-blue-400' : 'text-slate-400'}`} />
              </button>
              ...
              {servicesOpen && (
                <div className="absolute top-full left-0 w-[420px] max-w-[calc(100vw-24px)] pt-2 animate-in fade-in slide-in-from-top-1 duration-150 z-50">
                  <div className="p-3 bg-white border border-slate-200 rounded-2xl shadow-2xl shadow-slate-900/10 grid grid-cols-2 gap-2 text-slate-900">
            ...
            >
              <Link 
                href="/about" 
                className={`flex items-center gap-1 px-2.5 xl:px-3 py-1.5 rounded-xl transition-all duration-200 ${
                  pathname.startsWith('/about')
                    ? isAboutPage
                      ? 'bg-[#3B82F6] text-white font-bold shadow-sm border border-blue-400/40'
                      : 'bg-white text-violet-700 shadow-sm font-bold border border-slate-200/60'
                    : isAboutPage
                      ? 'text-slate-300 hover:text-white hover:bg-blue-900/50'
                      : 'text-slate-600 hover:text-slate-900 hover:bg-white/60'
              ...
              >
                <span>About Us</span>
                <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${aboutOpen ? 'rotate-180 text-blue-300' : 'text-slate-400'}`} />
              </Link>

              {aboutOpen && (
                <div className="absolute top-full left-0 w-64 pt-2 animate-in fade-in slide-in-from-top-1 duration-150 z-50">
                  <div className="p-2 bg-white border border-slate-200 rounded-2xl shadow-xl shadow-slate-900/10 space-y-1 text-xs text-slate-900">
                    <Link 
                      href="/about#story" 
                      className="p-2.5 rounded-xl hover:bg-violet-50 transition flex items-center justify-between text-slate-800 hover:text-violet-900 font-semibold group"
                    ...
                    >
                      ...
                      <ArrowRight className="w-3.5 h-3.5 text-slate-400 group-hover:translate-x-1 group-hover:text-violet-600 transition-transform" />
                    </Link>
          ...
          <div className="hidden lg:flex items-center gap-2 shrink-0">
            <Link
              href="/calculator"
              className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-xl border transition shadow-2xs ${
                isAboutPage
                  ? 'border-blue-500/40 text-slate-200 hover:text-white hover:border-blue-400 bg-blue-950/60'
                  : 'border-slate-200 text-slate-700 hover:text-violet-700 hover:bg-slate-50'
              }`}
            >
              <Calculator className={`w-3.5 h-3.5 ${isAboutPage ? 'text-blue-400' : 'text-violet-600'}`} />
          ...
          <div className="flex lg:hidden items-center gap-1.5 shrink-0">
            ...
            <button
              ...
              className={`p-2 rounded-xl border active:scale-95 transition cursor-pointer min-w-[38px] min-h-[38px] flex items-center justify-center ${
                isAboutPage
                  ? 'bg-blue-950/80 border-blue-800/60 text-white'
